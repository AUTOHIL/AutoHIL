...
from genAbstract import generate_abstract_en


SUMMARY_CACHE_PATH = "./abstract/summary_cache_en_openai.json"

class FuncNode:
    def __init__(self, name, file, line_start, line_end, code, is_atomic):
        self.name = name
        self.file = file
...
def build_call_forest_with_references(edges, function_info_map, dot_path=None):
    ...
    for func_name in function_info_map:
        if func_name not in all_involved_funcs:
            ...
            print(f"Unexpected function! -- {func_name}")

    if dot_file:
        dot_file.write("}\n")
        dot_file.close()
        print(f"Call graph DOT file writing completed: {dot_path}")

    return graph

# -- Update node flags --
# 1. is_root: Is it a root node (no callers)
# 2. is_leaf: Is it a leaf node (no callees)
# 3. is_visited: Is it visited (for traversal)
...
# ---------- Reverse Topological Sort (Bottom-up) ----------
def traverse_bottom_up(graph):
    # Record out-degree of each node (who calls it)
    ...
    while queue:
        ...
        for parent in current.callers:
            # Each parent only enters the next level after all its children are processed
...
def write_traverse_paths_to_file(levels, max_level, output_path):
    with open(output_path, "w") as f:
        for level in range(max_level + 1):
            ...
            if level != max_level:
                f.write("↑\n\n")
    print(f"Bottom-up level traversal results written to: {output_path}")
...
def detect_cycles(graph):
    ...
    def dfs(node, path):
        if node.name in rec_stack:
            # Cycle found, record the path of the cycle
...
def complete_levels_with_cycles(graph, levels, max_level):
    ...
    if missing_nodes:
        # Create a special level or output these nodes directly
...
# Generate summary based on function levels
def generate_abstract_from_levels(levels, max_level):
    # abstract_map = {}
    ...
    print(f"\n🚀 Starting summary generation, total {total_nodes} function nodes ({processed_nodes} already cached)")
    ...
    for level in range(max_level + 1):
        for node in levels[level]:
            ...
            abstract = generate_abstract_en(node.name, current_code, callee_abstracts)
            abstract_map[node.name] = abstract
            processed_nodes += 1
            print_progress_bar(processed_nodes, total_nodes)

            if node.is_atomic:
                atomic_api_map[node.name] = abstract
...
def save_abstract_map(abstract_map, output_path):
    ...
    print(f"Summary information written to: {output_path}")
...
if __name__ == "__main__":
    # Configure paths
    dot_directory = "./api_pool/callgraph"
    abstract_directory = "./abstract"
    ...
    abstract_map_path = os.path.join(abstract_directory, "api_abstracts_openai.json")
    ...
    graph = build_call_forest_with_references(
        dot_edges, 
        function_info_map,
        # dot_path=os.path.join(abstract_directory, "callForest.dot")
    )
    ...
    write_traverse_paths_to_file(levels, max_level+1, bottom_up_path)
    
    cycles = detect_cycles(graph)
    print(f"Cycles detected: {cycles}")

    abstract_map, atomic_api_map = generate_abstract_from_levels(levels, max_level)
    save_abstract_map(atomic_api_map, abstract_map_path)
